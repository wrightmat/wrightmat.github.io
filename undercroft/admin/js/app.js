...
// singular, since library_items is keyed by kind) — no more pluralized
// bucket-name-to-content-type translation needed. The old version's
// `bucket.endsWith("s") ? bucket.slice(0, -1) : bucket` fallback mangled any
// kind whose singular form doesn't end by just dropping an "s" (e.g.
// "species" -> "specie") — this sidesteps that entirely.
...
function ownedCharacters() {
  return viewState.owned.items.filter((item) => item.bucket === "character");
}
