...
- When adding or refactoring helper functions in the page scripts (for example `js/pages/workbench-template-view.js`), run a quick `rg function-name js/pages/workbench-template-view.js` search before introducing a new identifier. ES module parsing halts on duplicate `function` declarations, which prevents the editors from loading. Keeping names unique avoids the "Identifier has already been declared" console errors we've encountered.
