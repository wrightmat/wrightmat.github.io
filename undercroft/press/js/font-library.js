...
export const FONT_OPTIONS = [{ id: "default", label: "Default (theme font)", family: null }];
...
export async function saveCustomFont(font, authToken) {
  const response = await fetch("/press/custom-fonts", {
    method: "POST",
...
export async function saveCustomFontDeletion(id, authToken) {
  const response = await fetch("/press/custom-fonts/delete", {
    method: "POST",
    headers: authHeaders(authToken),
...
// through this server's own /press/google-fonts-metadata proxy (server/app.py)
...
const GOOGLE_FONTS_METADATA_URL = "/press/google-fonts-metadata";
