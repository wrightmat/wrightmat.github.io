...
1. **Extend the system schema to describe collections — done, now authored in Loom.**
   - `array` fields carry an `item` contract that mirrors `object` children (e.g., `{ type: "array", key: "inventory", item: { type: "object", displayField: "inventory[].name", children: [...] } }`), authored via Loom's Systems tab: an object property gets a recursive Sub-fields list, an array property is either Enum (a flat, System-defined value list) or Records (its own recursive Sub-fields list plus a Display field).【F:undercroft/loom/js/app.js†L1458-L1645】
