...
const ElementRenderer = {
  ...
  renderControlLabel( fieldName, alignment, extraClass = '', element = {} ) {
    const textAlignClass = element.textAlignment ? ` text-align-${element.textAlignment}` : '';
    const elementStyles = this.applyElementStyles(element, 'container');
    return `<label class="field-label ${textAlignClass} ${extraClass}" ${elementStyles}>${fieldName}</label>`;
  },
  ...
  renderContainer(element, context) {
    ...
    if ( showLabel ) {
    const textAlignClass = element.textAlignment ? ` text-align-${element.textAlignment}` : '';
        content += `<div class="group-label${textAlignClass}">${element.label}</div>`;
    }
  ...
  renderContainerGroup( element, context ) {
    ...
    const layoutClass = layout === 'grid' ? 'grid-layout' : 
                       ...
                       layout === 'compact-row' ? 'compact-row-layout' :
  ...
  renderTabs( element, context ) {
    ...
    element.elements.forEach((tab, index) => {
        ...
        content += `<div class="tab-header ${isActive ? 'active' : ''}" 
                         ...
                         onclick="Utils.switchTab('${element.id}', ${index},  window.editorInstance || window.characterAppInstance )"
  ...
  renderCircularTrack( element, context ) {
    ...
    const value = isPreview ? 2 : getValue(element.scope) || 0;
    let segmentsHtml = '';
    const centerX = 50;
    const centerY = 50;
    const radius = 35;
    for ( let i = 0; i < segments; i++ ) {
        ...
        const clickHandler = ( viewMode || isPreview ) ? '' : `onclick="window.characterAppInstance.updateTrackValue('${element.scope}', ${i + 1}, ${segments})"`;
