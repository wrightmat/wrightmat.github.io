class TemplateEditor {
	...
	this.property_definitions = {
	    ...
	    inputType: { type: 'select', label: 'Input Type', group: 'general',
	        options: [
	...
	this.element_definitions = {
	    'Basic Elements': [
		    ...
		    properties: ['label', 'scope', 'backgroundColor', 'textColor', 'textSize', 'textBold', 'width', 'layout', 'inputType', 'readOnly']
	        },
	        {
	            type: 'Array', icon: '📋',
	            label: 'Array/List',
		    ...
		    properties: ['label', 'scope', 'backgroundColor', 'width', 'displayStyle']
	        },
	        {
	            type: 'Divider', icon: '➖',
	            label: 'Divider',
		    ...
		    properties: ['label', 'textColor', 'textSize', 'textBold', 'textItalic', 'textUnderline']
	        }
	    ],
	    'Layout Elements': [
	        {
		    ...
		    properties: ['label', 'backgroundColor', 'borderColor', 'layout', 'width']
	        },
	        {
	            type: 'Tabs', icon: '📑',
	            label: 'Tabs',
	    ...
	    'Advanced Elements': [
		    ...
		    properties: ['label', 'scope', 'backgroundColor']
	        },
	        {
        ...
        header.innerHTML = `
            <div class="template-header-content">
                <input type="text" 
                       ...
                       value="${this.currentTemplate.title}" 
    ...
    createElement( evt ) {
        ...
        const definition = this.findElementDefinition(elementType);
    ...
    highlightSelectedElement() {
        ...
        if ( this.selectedElementId ) {
            const element = document.querySelector(`[data-element-id="${this.selectedElementId}"]`);
    ...
    renderPropertiesPanel( element, availableFields = [] ) {
        ...
        const propertyNames = elementDefinition && elementDefinition.properties ? elementDefinition.properties : ['label'];
    ...
    renderPropertyField( field, element, availableFields ) {
        ...
        switch ( field.type ) {
            case 'text':
                ...
                // Add autocomplete for scope fields
                if (field.hasAutocomplete && availableFields.length > 0) {
                    fieldHtml += `<div class="field-suggestions" id="${fieldId}-suggestions"></div>`;
                }
                break;
