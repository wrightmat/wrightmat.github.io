...
HT.Grid = function(/*double*/ width, /*double*/ height) {
	if ($.cookie('dnd_hex_grid') !== undefined) {
		this.Hexes = JSON.parse($.cookie('dnd_hex_grid'));
	} else {
		this.Hexes = [];
	}
...
function refreshHexGrid() {
	...
	var dnd_hex_grid = JSON.stringify(grid.Hexes);
	$.cookie('dnd_hex_grid', dnd_hex_grid);
console.log($.cookie('dnd_hex_grid'))
}

function addHexListeners() {
	var canvas = document.getElementById("hexCanvas");
